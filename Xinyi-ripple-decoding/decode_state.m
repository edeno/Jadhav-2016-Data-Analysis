...
for ripple_number = 1:length(ripple_index)
    ...
    %state transition
    stateM_Indicator_outbound = stateM_I_normalized_gaussian{1};
    stateM_Indicator_inbound = stateM_I_normalized_gaussian{2};
    ...
    for step_ind = 1:numSteps       
        ...
        if isempty(aa) %if no spike occurs at time t
            L_I0 = exp(-Lint_I_Lambda{1} .* dt);
            L_I1 = exp(-Lint_I_Lambda{1} .* dt);
            L_I2 = exp(-Lint_I_Lambda{2} .* dt);
            L_I3 = exp(-Lint_I_Lambda{2} .* dt);

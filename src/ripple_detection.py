...
def _get_candidate_ripples_Kay(filtered_lfps, is_multitaper=False, minimum_duration=0.015,
    ...
    '''Candidate ripple times are extracted based on mean power on all tetrodes

    Using power from all tetrodes reflects that we view the ripple as a population event
    ...
    minimum_duration : float, optional
        Minimum time the z-score has to stay above threshold to be considered a
        candidate ripple
    zscore_threshold : float, optional
        Number of standard deviations the ripple power must exceed to be considered
        a candidate ripple
...
def _get_candidate_ripples_Karlsson(filtered_lfps, minimum_duration=0.015,
                                    zscore_threshold=2):
    '''Candidate ripple times are detected on each tetrode and then combined
    if they overlap
...
def _extend_threshold_to_mean(is_above_mean, is_above_threshold, minimum_duration=0.015):
    ...
    is_above_threshold : Pandas series
        Time series indicator function specifying when the
        time series is above the the threshold. Index of the series is time.
...
def _find_containing_interval(interval_candidates, target_interval):
    ...
    This is accomplished by finding the closest start time
...
def _extend_segment(segments_to_extend, containing_segments):
    '''Extends the boundaries of a segment if it is
    a subset of one of the containing segments
...
def _smooth(data, sigma, sampling_frequency, axis=0, truncate=8):
    ...
    The standard deviation of the gaussian is in the units of the sampling frequency.
    The function is just a wrapper around scipy's gaussian_filter1d,
...
def _threshold_by_zscore(data, zscore_threshold=2):
    '''Standardize the data and determine whether it is above a given number
    ...
    threshold_dataframe : Pandas dataframe
        ...
        One column is an indicator function where 1 indicates the z-score
        is above the mean (z-score = 0) and 0 indicates the z-score is below the mean
